...
const GLOBAL_DOC_TEMPLATES: Array<{ name: string; discipline: string; content: string; linkedChecklistIds: number[] }> = [
  ...
  // ── PROPERTY MANAGER ───────────────────────────────────────────────────────
  {
    ...
    content: `<div style="font-family:Arial,sans-serif;max-width:800px;margin:0 auto;padding:40px;">
  <div style="background:#0e7490;color:#fff;padding:24px 32px;display:flex;justify-content:space-between;align-items:center;border-radius:8px 8px 0 0;">
    <div>
      <div style="font-size:22px;font-weight:700;letter-spacing:1px;">{{company_name}}</div>
      <div style="font-size:11px;color:#a5f3fc;margin-top:4px;letter-spacing:0.5px;">Property Management Services · ABN {{abn}}</div>
    </div>
    <div style="text-align:right;font-size:11px;color:#cffafe;">
      <div>{{company_address}}</div>
      <div>{{phone}} · {{email}}</div>
    </div>
  </div>
  <div style="border:2px solid #0e7490;padding:18px 32px;background:#ecfeff;">
    <div style="font-size:18px;font-weight:700;color:#0B1933;letter-spacing:1px;text-align:center;text-transform:uppercase;">Property Condition Report</div>
    <div style="text-align:center;font-size:11px;color:#0e7490;margin-top:4px;">Routine Inspection · {{inspection_date}}</div>
  </div>
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;width:40%;">Property Address</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{property_address}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Inspection Date</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspection_date}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Inspector</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspector_name}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Tenant</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{tenant_name}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Inspection Type</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">Routine Property Inspection</td></tr>
    </table>
    <div style="background:#ecfeff;border-left:4px solid #0e7490;padding:12px 16px;margin-bottom:24px;border-radius:0 4px 4px 0;">
      <div style="font-size:13px;font-weight:600;color:#0B1933;">Overall Property Condition</div>
      <div style="font-size:22px;font-weight:800;color:#0e7490;margin-top:4px;">{{overall_result}}</div>
      <div style="font-size:11px;color:#6b7280;margin-top:4px;">{{pass_count}} items satisfactory · {{fail_count}} items requiring attention</div>
    </div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #0e7490;padding-bottom:4px;">Inspection Summary</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{summary_notes}}</div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #0e7490;padding-bottom:4px;">Checklist Results</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{checklist_results}}</div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #0e7490;padding-bottom:4px;">Maintenance Items Identified</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{defect_list}}</div>
    <div style="margin-top:36px;display:flex;justify-content:space-between;">
      <div>
        <div style="border-top:1px solid #374151;width:220px;margin-bottom:4px;"></div>
        <div style="font-size:11px;color:#6b7280;">Property Manager Signature</div>
        <div style="font-size:12px;font-weight:600;color:#0B1933;margin-top:2px;">{{inspector_name}}</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:11px;color:#6b7280;">Date</div>
  ...
  <div style="background:#0e7490;color:#a5f3fc;font-size:10px;padding:10px 32px;text-align:center;border-radius:0 0 8px 8px;">
    This document was generated by InspectProof · {{company_name}} · Property Management
  </div>
</div>`,
    linkedChecklistIds: [],
  },
  // ── POOL INSPECTOR ─────────────────────────────────────────────────────────
  {
    name: "Pool Safety Compliance Certificate",
    discipline: "Pool Inspector",
    content: `<div style="font-family:Arial,sans-serif;max-width:800px;margin:0 auto;padding:40px;">
  <div style="background:#075985;color:#fff;padding:24px 32px;display:flex;justify-content:space-between;align-items:center;border-radius:8px 8px 0 0;">
    <div>
      <div style="font-size:22px;font-weight:700;letter-spacing:1px;">{{company_name}}</div>
      <div style="font-size:11px;color:#7dd3fc;margin-top:4px;letter-spacing:0.5px;">Licensed Pool Inspector · ABN {{abn}}</div>
    </div>
    <div style="text-align:right;font-size:11px;color:#bae6fd;">
      <div>{{company_address}}</div>
      <div>{{phone}} · {{email}}</div>
      <div>Licence No: {{license_number}}</div>
    </div>
  </div>
  <div style="border:2px solid #075985;padding:18px 32px;background:#e0f2fe;">
    <div style="font-size:18px;font-weight:700;color:#075985;letter-spacing:1px;text-align:center;text-transform:uppercase;">Pool Safety Compliance Certificate</div>
    <div style="text-align:center;font-size:11px;color:#0369a1;margin-top:4px;">Pool Barrier &amp; Safety Inspection · {{inspection_date}}</div>
  </div>
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;width:40%;">Property Address</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{property_address}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Inspection Date</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspection_date}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Pool Inspector</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspector_name}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Pool Type</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{pool_type}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Certificate Number</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{certificate_number}}</td></tr>
    </table>
    <div style="background:#e0f2fe;border-left:4px solid #075985;padding:12px 16px;margin-bottom:24px;border-radius:0 4px 4px 0;">
      <div style="font-size:13px;font-weight:600;color:#0B1933;">Compliance Outcome</div>
      <div style="font-size:22px;font-weight:800;color:#075985;margin-top:4px;">{{overall_result}}</div>
      <div style="font-size:11px;color:#6b7280;margin-top:4px;">{{pass_count}} items compliant · {{fail_count}} items non-compliant</div>
    </div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #075985;padding-bottom:4px;">Barrier &amp; Fence Assessment</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{checklist_results}}</div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #075985;padding-bottom:4px;">Non-Compliant Items</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{defect_list}}</div>
    <div style="font-size:11px;color:#6b7280;background:#f8fafc;padding:12px 16px;border-radius:4px;margin-bottom:20px;">
      This certificate is issued pursuant to the relevant state Swimming Pools Act. A re-inspection is required within the timeframe specified above if any non-compliant items are identified.
    </div>
    <div style="margin-top:36px;display:flex;justify-content:space-between;">
      <div>
        <div style="border-top:1px solid #374151;width:220px;margin-bottom:4px;"></div>
        <div style="font-size:11px;color:#6b7280;">Pool Inspector Signature</div>
        <div style="font-size:12px;font-weight:600;color:#0B1933;margin-top:2px;">{{inspector_name}}</div>
        <div style="font-size:11px;color:#6b7280;">Licence: {{license_number}}</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:11px;color:#6b7280;">Certificate Date</div>
        <div style="font-size:13px;font-weight:600;color:#0B1933;margin-top:2px;">{{inspection_date}}</div>
      </div>
    </div>
  </div>
  <div style="background:#075985;color:#7dd3fc;font-size:10px;padding:10px 32px;text-align:center;border-radius:0 0 8px 8px;">
    This document was generated by InspectProof · {{company_name}} · Pool Safety Inspection Services
  </div>
</div>`,
    linkedChecklistIds: [],
  },
  // ── INSURANCE ASSESSOR ─────────────────────────────────────────────────────
  {
    ...
    discipline: "Insurance Assessor",
    content: `<div style="font-family:Arial,sans-serif;max-width:800px;margin:0 auto;padding:40px;">
  <div style="background:#3730a3;color:#fff;padding:24px 32px;display:flex;justify-content:space-between;align-items:center;border-radius:8px 8px 0 0;">
    <div>
      <div style="font-size:22px;font-weight:700;letter-spacing:1px;">{{company_name}}</div>
      <div style="font-size:11px;color:#a5b4fc;margin-top:4px;letter-spacing:0.5px;">Insurance Assessment Services · ABN {{abn}}</div>
    </div>
    <div style="text-align:right;font-size:11px;color:#c7d2fe;">
      <div>{{company_address}}</div>
      <div>{{phone}} · {{email}}</div>
      <div>Assessor Licence: {{license_number}}</div>
    </div>
  </div>
  <div style="border:2px solid #3730a3;padding:18px 32px;background:#eef2ff;">
    <div style="font-size:18px;font-weight:700;color:#3730a3;letter-spacing:1px;text-align:center;text-transform:uppercase;">Insurance Assessment Report</div>
    <div style="text-align:center;font-size:11px;color:#4338ca;margin-top:4px;">Property Damage Assessment · {{inspection_date}}</div>
  </div>
  <div style="padding:28px 32px;background:#fff;border:1px solid #e5e7eb;border-top:none;">
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;width:40%;">Property Address</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{property_address}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Date of Loss</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{date_of_loss}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Assessment Date</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspection_date}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Assessor</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{inspector_name}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Claim Reference</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{claim_reference}}</td></tr>
      <tr><td style="padding:8px;background:#f8fafc;font-weight:600;color:#0B1933;font-size:13px;">Loss Type</td><td style="padding:8px;font-size:13px;border-bottom:1px solid #f1f5f9;">{{loss_type}}</td></tr>
    </table>
    <div style="background:#eef2ff;border-left:4px solid #3730a3;padding:12px 16px;margin-bottom:24px;border-radius:0 4px 4px 0;">
      <div style="font-size:13px;font-weight:600;color:#0B1933;">Assessment Summary</div>
      <div style="font-size:22px;font-weight:800;color:#3730a3;margin-top:4px;">{{overall_result}}</div>
      <div style="font-size:11px;color:#6b7280;margin-top:4px;">{{pass_count}} items assessed · {{fail_count}} items with identified damage</div>
    </div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #3730a3;padding-bottom:4px;">Damage Assessment Details</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{checklist_results}}</div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #3730a3;padding-bottom:4px;">Identified Damage Items</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{defect_list}}</div>
    <div style="font-size:13px;font-weight:700;color:#0B1933;margin-bottom:8px;border-bottom:2px solid #3730a3;padding-bottom:4px;">Assessor Notes</div>
    <div style="font-size:13px;color:#374151;margin-bottom:20px;">{{summary_notes}}</div>
    <div style="margin-top:36px;display:flex;justify-content:space-between;">
      <div>
        <div style="border-top:1px solid #374151;width:220px;margin-bottom:4px;"></div>
        <div style="font-size:11px;color:#6b7280;">Insurance Assessor Signature</div>
        <div style="font-size:12px;font-weight:600;color:#0B1933;margin-top:2px;">{{inspector_name}}</div>
        <div style="font-size:11px;color:#6b7280;">Licence: {{license_number}}</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:11px;color:#6b7280;">Assessment Date</div>
        <div style="font-size:13px;font-weight:600;color:#0B1933;margin-top:2px;">{{inspection_date}}</div>
      </div>
    </div>
  </div>
  <div style="background:#3730a3;color:#a5b4fc;font-size:10px;padding:10px 32px;text-align:center;border-radius:0 0 8px 8px;">
    This document was generated by InspectProof · {{company_name}} · Insurance Assessment Services
  </div>
</div>`,
    linkedChecklistIds: [],
  },

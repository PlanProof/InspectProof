...
export async function ensureGlobalTemplatesSeed(): Promise<void> {
  try {
    ...
    if (existingDocs.length === 0) {
      console.log("[Seed] No doc templates found — seeding global doc templates...");
      for (const tmpl of GLOBAL_DOC_TEMPLATES) {
        ...
        console.log(`  ✓ Doc template: ${tmpl.name}`);
      }
      console.log("[Seed] Doc templates seeded successfully");
    } else {
      console.log("[Seed] Doc templates already exist — skipping doc seed");
    }
  } catch (err) {
